...
@app.command()
def init(
    provider: str = typer.Option(..., "--provider", "-p", help="LLM provider (e.g., 'openai', 'gemini')"),
    api_key: str = typer.Option(..., "--api-key", "-k", help="Your API key for the provider"),
    model: str = typer.Option(None, "--model", "-m", help="The specific model to use (optional)"),
):
    """
    Initialize the Forge agent with your LLM provider and API key.
    """
    if ForgeConfig.CONFIG_FILE.exists():
        console.print("[yellow]Configuration already exists. To re-initialize, please run 'forge stop' first.[/yellow]")
        raise typer.Exit()
...
def _run_chat_repl(thread_id: str | None):
    ...
    try:
        ...
        console.print("[bold red]Configuration not found. Please run 'forge init' first.[/bold red]")
        raise typer.Exit(1)

    if not thread_id:
        thread_id = str(uuid.uuid4())
        console.print(Markdown(f"**New chat session started. Your Thread ID is:** `{thread_id}`"))
        console.print("You can use this ID with the main command to continue this conversation later.")
    ...
    with SqliteSaver.from_conn_string(str(ForgeConfig.CONFIG_DB)) as checkpointer:
        ...
        while True:
            try:
                ...
                console.print(f"[bold green]Agent:[/bold green] {agent_response.content}")
    ...
    if typer.confirm("Do you want to clear all conversation memory?"):
        ...
        console.print("[bold green]✅ All conversation memory has been cleared.[/bold green]")
    
    if typer.confirm("Do you want to delete all Forge data (config and all memory)?"):
        ForgeConfig.delete()
...
def main(
    ctx: typer.Context,
    thread_id: str = typer.Option(None, "--thread-id", "-t", help="Continue a conversation with a specific thread ID."),
):
    """
    Forge AI Agent CLI.
...
def stop():
    ...
    if ForgeConfig.CONFIG_DIR.exists():
        if typer.confirm("Are you sure you want to delete the entire .forge directory? This is irreversible."):
            ForgeConfig.delete()
            console.print("[bold green]✅ Forge configuration and memory have been reset.[/bold green]")
        else:
            console.print("Operation cancelled.")
    else:
        console.print("Nothing to stop. The forge does not exist.")
